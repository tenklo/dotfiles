...
(customize-set-variable 'desktop-load-locked-desktop t)
...
(defun kk/mu4e-set-split ()
  (let* ((horizontal (when (< (frame-text-height) (frame-text-width))))
         (split (if horizontal 'vertical 'horizontal)))
    (setq mu4e-split-view split)))

(add-hook 'mu4e-view-mode-hook #'kk/mu4e-set-split)
#+end_src
