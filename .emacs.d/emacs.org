...
#+begin_src emacs-lisp
  ...
  ;;   ((lambda () (add-to-list 'mu4e-bookmarks '("m:/kloess@digital-worx.de/Inbox or m:/karsten.kloess@its-stuttgart.de/Inbox" "All Inboxes" ?a)))
  ;;    (lambda () (add-to-list 'mu4e-bookmarks '("m:/kloess@digital-worx.de/Inbox AND flag:unread or m:/karsten.kloess@its-stuttgart.de/Inbox AND flag:unread" "All Unread" ?s)))))
          ...
          (list
           ...
           (make-mu4e-context
            ...
            :vars '((user-mail-address . "kloess@digital-worx.de")
                    ...
                    (smtpmail-smtp-server . "mail.digital-worx.de")
                    ...
                    (mu4e-bookmarks .
                                    ((:name "Unread digital-worx" :query "m:/kloess@digital-worx.de/ AND flag:unread AND NOT flag:trashed" :key ?u)
                                     (:name "Inbox digital-worx" :query "m:/kloess@digital-worx.de/Inbox" :key ?i)
                                     ...
                                     (:name "Messages with images digital-worx" :query "mime:image/*" :key ?p)))))
            ...
            :vars '((user-mail-address . "karsten.kloess@its-stuttgart.de")
                    ...
                    (mu4e-bookmarks .
                                    ((:name "Unread Schule" :query "m:/karsten.kloess@its-stuttgart.de/ AND flag:unread AND NOT flag:trashed" :key ?u)
                                     (:name "Inbox Schule" :query "m:/karsten.kloess@its-stuttgart.de/Inbox" :key ?i)
                                     (:name "Today's messages Schule" :query "date:today..now" :key ?t)
                                     (:name "Last 7 days Schule" :query "date:7d..now" :hide-unread t :key ?w)
                                     (:name "Messages with images Schule" :query "mime:image/*" :key ?p))))))))
  ;; :leave-func '(kk/add-standard-bookmarks)))))
...
Here, device specific configurations shall be declared. As soon as we know how.

This is a ugly work around regarding the font size, until we know more.

** DWHPD

#+begin_src emacs-lisp

  (when (equal system-name "dwhpd")
