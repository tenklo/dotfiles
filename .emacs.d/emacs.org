...
(use-package tab-bar
  ...
  :custom
  ((tab-bar-show t)
   (tab-bar-format '(kk/tab-bar-format-menu-bar tab-bar-format-tabs tab-bar-separator tab-bar-format-align-right tab-bar-format-global))
      ...
      (let ((repetitions (if ARG ARG 1)))
        (while (< 0 repetitions)
          (let* ((old-tags (org-get-tags nil 'only-local))
                 (person (car (kk/org-contacts-get-contact)))
...
(use-package bongo
  :commands
  (bongo bongo-playlist-buffer bongo-dired-library-mode kk/bongo-enqueue-uri)
  :custom
