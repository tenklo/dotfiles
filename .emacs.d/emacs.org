...
#+begin_src emacs-lisp
  ...
  (defun kk/mu4e-add-standard-bookmarks ()
    (interactive)
    (add-to-list 'mu4e-bookmarks '(:query "m:/kloess@digital-worx.de/Inbox or m:/karsten.kloess@its-stuttgart.de/Inbox" :name "All Inboxes" :key ?y) t)
    (add-to-list 'mu4e-bookmarks '(:query "flag:unread" :name "All Unread" :key ?x) t))

  (advice-add 'mu4e-context-switch
              :after #'kk/mu4e-add-standard-bookmarks)
