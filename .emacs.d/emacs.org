...
(defun kk/join-line-to-following ()
  (interactive)
  (join-line '(4)))
(define-key edit-commands-map (kbd "J") 'kk/join-line-to-following)
(define-key edit-commands-map (kbd "C-J") 'kk/join-line-to-following)
...
(use-package tab-bar
  ...
  (keymap-global-set "C-M-S-s-t" tab-prefix-map))
#+end_src
...
(defvar kk/work-default-task-id "8a1e79ac-b13f-48d5-a11d-da5b88fdc6cd")
...
(defun kk/org-agenda-auto-exclude-function (tag)
  (pcase tag
    ("HOLD"
     (concat "-" tag))
    ("WORK"
     (when (kk/org-punched-in-p)
       (concat "+" tag)))
    ("PERSONAL"
     (when (not (kk/org-punched-in-p))
       (concat "+" tag)))))
#+end_src
...
(setq org-todo-keywords
      '((sequence "TODO(t!/!)" "NEXT(n!/!)" "|" "DONE(d!/!)")
        (sequence "WAIT(w!/!)" "HOLD(h!/!)" "|" "CANC(c@/!)" "PHONE" "MEETING")))
...
(global-set-key (kbd "C-<f10>") 'kk/org-clock-goto-list)
...
(defun kk/clock-in-at-time (&optional time)
  ...
  (pcase major-mode
    ...
    ('org-mode
     (org-clock-in nil (org-read-date t t)))
    (_
     (error "Not in an Org(-Agenda)-Mode."))))
...
(define-key org-mode-map (kbd "C-c w") 'kk/org-link-copy)
#+end_src
...
#+begin_src emacs-lisp
(defun kk/org-clock-work-report (&optional arg)
  (interactive "P")
  (let ((org-clock-clocktable-default-properties `(:maxlevel 1 :lang "de" :scope agenda-with-archives :block ,(if arg 'week 'today) :fileskip0 t :match "WORK-pets" :narrow 40! :hidefiles t)))
    (org-clock-report)))
#+end_src
...
#+begin_src emacs-lisp
(keymap-global-set "C-M-S-s-SPC" '(lambda ()
                                    (interactive)
                                    (org-agenda nil " ")))
     ...
     (if (not (listp nmins))
       ...
       (let ((counter 0)
         ...
         (while (< counter length)
           (notifications-notify :title (car (s-split "  " (nth counter msg)))
                                 :body (format "In %d minutes." (string-to-number (nth counter nmins)))
                                 :app-name "Emacs: Org"
                                 :sound-name "alarm-clock-elapsed")
           (setq counter (1+ counter)))))
     (when (kk/alarm-should-ring-p nmins msg)
       (kk/alarm-ring))))
  :config
  (run-with-timer 5 1200 (lambda() (org-agenda-to-appt t)))

  (defvar kk/alarm-snooze-time 1
    "Snooze time in minutes.")
  (defvar kk/alarm-timer nil
    "Timer object.")

  (defun kk/alarm-ding (&rest rest)
    "Play ding.
  ...
  Ingores `rest'."
    (let ((title "Alarm Clock")
          (program (cond ((eq system-type 'darwin) "afplay")
                         ...
                         (t "")))
          (sound (expand-file-name kk/alarm-sound-file)))
      (when (and (executable-find program)
                 (file-exists-p sound))
        (start-process title nil program sound))))

  (defun kk/alarm-ring ()
    (cancel-timer kk/alarm-timer)
    (start-process "Volume" nil "amixer" "set" "Master" "100%")
    (unwind-protect
        (while (sit-for 3)
          (message "Wake up! To stop this madness, press a movement command.")
          (kk/alarm-ding))
      (let ((snooze (completing-read "Whats happening now? "
                                     '("snooze" "done")
                                     nil
                                     t)))
        (if (equal snooze "snooze")
            (progn
              (setq kk/alarm-timer (run-with-timer (* 60 kk/alarm-snooze-time) 1000 #'kk/alarm-ring))
              (message "Making noise again in %s minutes!" kk/alarm-snooze-time))
          (message "Have a nice day!")))))

  (defun kk/alarm-snooze-cancel ()
    (interactive)
    (cancel-timer kk/alarm-timer))

  (defun kk/alarm-should-ring-p (nmins msg)
    (let ((should-ring nil)
          (contains-keyword nil))
      (if (listp msg)
          (let ((counter 0))
            (mapcar (lambda (x)
                      (when (s-contains-p "WECKER" x)
                        (setq contains-keyword t))
                      (unless contains-keyword
                        (setq counter (1+ counter)))) msg)
            (when contains-keyword
              (when (equal "0" (nth counter nmins))
                (setq should-ring t))))
        (when (and (s-contains-p "WECKER" msg)
                   (equal "0" nmins))
          (setq should-ring t)))
      should-ring)))
...
(defun kk/project-task-where-project-is-done-p ()
  ...
  (let ((task (save-excursion (org-back-to-heading 'invisible-ok)
                              (point))))
    (save-excursion
      (when (not (equal (bh/find-project-task) task))
    ...
    (save-excursion
      ...
      (let* ((tags (vulpea-buffer-tags-get))
        ...
        (if (vulpea-agenda-file-p)
            (setq tags (cons "agendaf" tags))
          (setq tags (remove "agendaf" tags)))
...
(defun vulpea-agenda-file-files ()
  "Return a list of note files containing 'agendaf' tag." ;
  (seq-uniq
   (seq-map
    #'car
    (org-roam-db-query
      ...
      (let ((repetitions (if ARG ARG 1)))
        (while (< 0 repetitions)
          (let* ((old-tags (org-get-tags nil 'only-local))
            ...
            (org-set-tags (cl-remove-duplicates new-tags :test 'string=)))
...
#+begin_src emacs-lisp
(setq kk/weekly-review-personal-or-work nil)
...
(defun kk/weekly-review-auto-scope ()
  (interactive)
  (setq kk/weekly-review-personal-or-work nil)
  (kk/weekly-review-current-state-message))
  ...
  (message (concat "Currently reviewing "
                   (pcase kk/weekly-review-personal-or-work
                     ...
                     ('all
                      "all")
                     (_
                      "according to punched-in status")))))
...
(defun kk/weekly-review-projects ()
  (interactive)
  (let ((org-agenda-sticky nil)
        (org-agenda-tag-filter-preset
         '("-HOLD"))
...
#+begin_src emacs-lisp
(defun kk/org-reschedule-on-done ()
  "Reschedule task at point when its state is done and its property RESCHEDULE_ON_DONE is non-nil.

Also, when SMART_RESCHEDULE_ON_DONE is non-nil, reschedule using `org-smart-reschedule'.

Meant to be used as `org-after-todo-state-change-hook'."
  (remove-hook 'post-command-hook 'org-add-log-note)
  (when (member org-state org-done-keywords)
    (let ((id (org-id-get))
          (org-log-reschedule 'time))
      (when (org-not-nil (org-entry-get (point) "RESCHEDULE_ON_DONE"))
        (call-interactively #'org-schedule)
        (org-todo "TODO"))
      (when (org-not-nil (org-entry-get (point) "SMART_RESCHEDULE_ON_DONE"))
        (call-interactively #'org-smart-reschedule)
        (org-todo "TODO"))))
  (add-hook 'post-command-hook 'org-add-log-note))

(add-to-list 'org-default-properties "RESCHEDULE_ON_DONE")
(add-to-list 'org-default-properties "SMART_RESCHEDULE_ON_DONE")

(add-hook 'org-after-todo-state-change-hook #'kk/org-reschedule-on-done)
#+end_src

** Org-Drill

#+begin_src emacs-lisp
(defun kk/enlarge-echo-area (original &optional args)
  "Enlarges the max echo area size to accomodate for multi line output."
  (let ((max-mini-window-height 0.5))
    (funcall original args)))

(use-package org-drill
  :custom
  (org-drill-adjust-intervals-for-early-and-late-repetitions-p t)
  :config
  (advice-add 'org-drill :around 'kk/enlarge-echo-area)

  (defun kk/org-drill-agenda (&optional with-archives)
    (interactive "P")
    (org-drill (if with-archives 'agenda-with-archives 'agenda))))
#+end_src
...
(use-package magit
  ...
  :config
  (defun kk/jump-to-commit ()
    (interactive)
    (let* ((chunk (magit-current-blame-chunk 'addition))
           (rev (slot-value chunk 'orig-rev)))
...
** Neotree
#+begin_src emacs-lisp
(define-key edit-commands-map (kbd "d") 'neotree-toggle)
(use-package neotree
...
#+begin_src emacs-lisp
(winner-mode)
(keymap-global-set "C-M-S-s-<left>" #'winner-undo)
(keymap-global-set "C-M-S-s-<right>" #'winner-redo)
#+end_src
...
(keymap-global-set "C-M-S-s-o" #'delete-other-windows)
(keymap-global-set "C-M-S-s-i" #'delete-window)
#+end_src
...
(defhydra bongo-hydra (:hint nil)
  "
Bongo-Control                                                ^Buffers
^^^^^^^^-----------------------------------------------------------------
_SPC_: pause/resume       _n_: next           _S_: seek      _m_: music
_s_:   start/stop         _p_: previous       ^ ^            _o_: other
"
  ...
  ("S" bongo-seek)
  ("b" bongo :exit t)
  ("m" kk/bongo-open-music-buffer :exit t)
  ("o" kk/bongo-open-other-media-buffer :exit t)
  ("q" nil :exit t))
...
"
  ...
  ("p" kk/weekly-review-projects :exit t)
...
Taken from [[https://www.reddit.com/r/emacs/comments/kbwkca/compile_buffer_show_weird_symbols/][here]].
...
#+begin_src emacs-lisp
(defhydra hydra-caesar ()
  "Caesar Cipher"
  ("f" (lambda () (interactive) (message-caesar-buffer-body 1)) "Shift Forward buffer")
  ("b" (lambda () (interactive) (message-caesar-buffer-body -1)) "Shift Backward buffer"))
#+end_src
...
#+begin_src emacs-lisp
(use-package logos
  :commands (logos-focus-mode logos-narrow-dwim)
  :custom
...
:END:
#+begin_src emacs-lisp
(defun kk/tidy-buffer (&optional arg)
...
Unless `arg', try formatting with `eglot-format' before using `indent-region'."
  (interactive)
  (save-excursion
    (or (unless arg
          (condition-case nil
...
#+begin_src emacs-lisp
(defun kk/work-day-p (date)
  "Return non-nil when `date' is on a work day according to `kk/work-days'."
  (let ((entry t))
    (kk/work-days)))

(defun kk/count-work-days-between (date1 date2)
  "Count work days between `date1' and `date2'."
  (let ((start-date (if (< date1 date2) date1 date2))
        (end-date (if (> date1 date2) date1 date2))
        (work-days 0))
    (while (<= start-date end-date)
      (when (kk/work-day-p (calendar-gregorian-from-absolute start-date))
        (cl-incf work-days))
      (cl-incf start-date))
    work-days))

(defun kk/count-days (d1 d2)
  (let* ((days (- (calendar-absolute-from-gregorian d1)
                  (calendar-absolute-from-gregorian d2)))
         (days (1+ (if (> days 0) days (- days)))))
    days))

(defun kk/count-non-work-days-between (date1 date2)
  ...
  (let* ((d1 (calendar-absolute-from-gregorian date1))
         ...
         (work-days (kk/count-work-days-between d1 d2))
         (days (kk/count-days date1 date2))
         (diff (- days work-days)))
    diff))

(defun kk/calendar-count-non-work-days-region ()
  (interactive)
  (let* ((date1 (calendar-cursor-to-date t))
         (date2 (or (car calendar-mark-ring)
                    (error "No mark set in this buffer")))
         (non-work-days (kk/count-non-work-days-between date1 date2)))
...
(defun kk/calendar-count-work-days-region ()
  ...
  (let* ((date1 (calendar-absolute-from-gregorian
         ...
         (date2 (calendar-absolute-from-gregorian
                 (or (car calendar-mark-ring)
                     (error "No mark set in this buffer"))))
         (work-days (kk/count-work-days-between date1 date2)))
...
(defun kk/calendar-count-days-region (&optional arg)
  (interactive "P")
  (pcase arg
    ('nil
     (calendar-count-days-region))
    ('(4)
     (kk/calendar-count-work-days-region))
    ('(16)
     (kk/calendar-count-non-work-days-region))))
(keymap-set calendar-mode-map "M-=" 'kk/calendar-count-days-region)
#+end_src

*** Holiday Plan Capturing

#+begin_src emacs-lisp
(defun kk/work-new-holidays-org-project-for (date1 date2)
  (let* ((day-count (kk/count-days date1 date2))
         ...
         (days-string-id (kk/yas-id-remove-spaces days-string)))
    (with-current-buffer (find-file-noselect kk/org-refile-target)
      (goto-char (point-max))
      (insert
       "* TODO " days-string "\n"
       ":PROPERTIES:\n"
       ...
       ":END:\n"
       "** TODO ggf. Eintrag in work-days.el\n" ; TODO: generate string to insert in work-days.el
       "** TODO Abwesenheitsbenachrichtigung Aktivierung schedulen\n"
       "** TODO Abwesenheitsbenachrichtigung aktivieren\n ")))) ; TODO: schedule to last possible non-work day

(defun kk/calendar-new-holidays ()
  (interactive)
  (let* ((date1 (calendar-cursor-to-date t))
         (date2 (or (car calendar-mark-ring)
                    (error "No mark set in this buffer"))))
    (kk/work-new-holidays-org-project-for date1 date2)))

(defun kk/work-new-holidays-org-project-for (date1 date2)
  (let* ((date1 (calendar-absolute-from-gregorian date1))
         (date2 (calendar-absolute-from-gregorian date2))
         (day-count (kk/count-non-weekend-or-official-holiday-days date1 date2))
         (start-date (if (< date1 date2) date1 date2))
         (end-date (if (> date1 date2) date1 date2))
         (days-string (if (equal day-count 1)
                          (format-time-string "%d. %B %Y" start-date)
...
(defun kk/is-official-holiday-p (date)
  (let ((calendar-holidays holiday-local-holidays))
    (calendar-check-holidays date)))

(defun kk/is-weekend-p (date)
  (memq
   (calendar-day-of-week date) calendar-weekend-days))

(defun kk/count-non-weekend-or-official-holiday-days (d1 d2)
  (let* ((tmp-date (if (< d1 d2) d1 d2))
    ...
    (while (<= tmp-date end-date)
      (let ((date-gregorian (calendar-gregorian-from-absolute tmp-date)))
        (when (and (not (kk/is-official-holiday-p date-gregorian))
                   (not (kk/is-weekend-p date-gregorian)))
          (cl-incf days))
        (cl-incf tmp-date)))
    days))

(defun kk/calendar-non-weekend-or-official-holiday-days ()
  (interactive)
  (let* ((date1 (calendar-absolute-from-gregorian
                 (calendar-cursor-to-date t)))
         (date2 (calendar-absolute-from-gregorian
                 (or (car calendar-mark-ring)
                     (error "No mark set in this buffer"))))
         (days (kk/count-non-weekend-or-official-holiday-days date1 date2)))
...
interfere with the default variable `bongo-playlist-buffer'."
  ...
  (defun kk/bongo-music-playlist ()
    (let ((buffer (get-buffer-create "*Bongo Playlist*")))
      (with-current-buffer buffer
        (when (not (equal major-mode 'bongo-playlist-mode))
          (bongo-playlist-mode)))
      buffer))

  (defun kk/bongo-open-music-buffer ()
    (interactive)
    (let ((bongo-playlist-buffer (kk/bongo-music-playlist)))
      (bongo-playlist))))
#+end_src
...
(defun kk/bongo-open-other-media-buffer ()
  (interactive)
  (let ((bong-playlist-buffer (prot-elfeed-bongo-playlist)))
    (bongo-playlist)))
#+end_src
...
(use-package anki-editor
  ...
  (anki-editor-ignored-org-tags '("export" "noexport" "agendaf" "learn"))
  :config
...
(keymap-global-set "C-M-S-s-s" 'shell-dwim)
#+end_src
